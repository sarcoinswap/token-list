...
// import sarcoinswapSchema from "@sarcoinswap/token-lists/schema/sarcoinswap.json";
import sarcoinswapSchema from "./schema.json"; // TODO: exports path
...
const validate = ajv.compile(sarcoinswapSchema);
...
const toBeValidLogo = async (token) => {
  ...
  const refersToLocalLogo =
    token.logoURI ===
    `https://tokens.sarcoinswap.finance/images${multiChainLogoPath?.[token.chainId] || ""}/${token.address}.png`;
  ...
  if (token.logoURI === `https://tokens.sarcoinswap.finance/images/symbol/${token.symbol.toLowerCase()}.png`) {
    return {
      message: () => ``,
      pass: true,
    };
